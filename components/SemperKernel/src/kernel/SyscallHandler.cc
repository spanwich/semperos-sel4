...
/**
 * Obtain/Delegate exchange protocol over a session (cf. Hille et al., USENIX ATC 2019, Section 4.2).
 *
 * Capability exchange uses a three-party protocol between the requesting VPE's
 * kernel (local), the service's kernel (remote, if cross-kernel), and the service.
 *
 * LOCAL SERVICE PATH (service and requester share a kernel):
 *
 *   VPE --OBTAIN/DELEGATE--> Kernel --fwd--> Service
 *                            Kernel <--reply-- Service
 *                            Kernel: do_exchange() clones caps, sets parent/child pointers
 *   VPE <---result---------- Kernel
 *
 *   The kernel subscribes a callback on the service gate, forwards the request
 *   to the service, and processes the reply asynchronously. The callback calls
 *   do_exchange() to clone capabilities and establish parent-child relationships.
 *
 * REMOTE SERVICE PATH (service on a different kernel):
 *
 *   VPE --OBTAIN/DELEGATE--> LocalKrnl --exchangeOverSession--> RemoteKrnl --fwd--> Service
 *                                                               RemoteKrnl <--reply-- Service
 *                                                               RemoteKrnl: add child ptrs (obtain)
 *                                                                           or reserve slots (delegate)
 *                            LocalKrnl <--exchangeOverSessionReply-- RemoteKrnl
 *                            LocalKrnl: thread wakes up, processes reply
 *
 *   For OBTAIN: RemoteKrnl adds child pointers to parent caps and serializes
 *     them. LocalKrnl deserializes, creates cap objects, and inserts via obtain().
 *     No ack needed — RemoteKrnl already inserted child pointers.
 *
 *   For DELEGATE (two-way handshake):
 *     1. RemoteKrnl reserves cap slots in service's CapTable, blocks thread
 *     2. LocalKrnl serializes the caps being delegated
 *     3. LocalKrnl sends exchangeOverSessionAck with cap data to RemoteKrnl
 *     4. RemoteKrnl wakes up, deserializes caps, inserts into service's CapTable
 *
 *   On failure: LocalKrnl sends removeChildCapPtr to clean up child pointers
 *     that were already added on RemoteKrnl.
 *
 * @param is      Incoming syscall message stream
 * @param obtain  true for obtain (service→requester), false for delegate (requester→service)
 */
    ...
    if(MHTInstance::getInstance().keyLocality(sess->obj->srvID)) { // local service
        ...
        if(res == m3::Errors::NO_ERROR) {
            ...
            if(obtain) {
                ...
                // Delegate: serialize local caps and send them to the remote kernel
                // so it can insert them into the service VPE's cap table.
                CapTable &srctab = caps.type() == m3::CapRngDesc::OBJ ? vpe->objcaps() : vpe->mapcaps();
                size_t capSerSize = 0;
                Capability *srcCaps[caps.count()];
                for(uint i = 0; i < caps.count(); i++) {
                    srcCaps[i] = srctab.get(caps.start() + i);
                    if(srcCaps[i])
                        capSerSize += Capability::serializedSizeTyped(srcCaps[i]);
                }

                AutoGateOStream capData(m3::vostreamsize(
                    m3::ostreamsize<uint>(), capSerSize));
                capData << caps.count();
                for(uint i = 0; i < caps.count(); i++) {
                    if(srcCaps[i])
                        Capability::serializeTyped(capData, srcCaps[i]);
                }

                Kernelcalls::get().exchangeOverSessionAck(Coordinator::get().getKPE(
                    MHTInstance::getInstance().responsibleMember(sess->obj->srvID)),
                    repliertid, m3::Errors::NO_ERROR, capData);
            }
