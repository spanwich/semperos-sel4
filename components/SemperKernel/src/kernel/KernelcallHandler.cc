...
//    VPE::self()._eps->set(DTU::KRNLC_EP);
    ...
    add_operation(Kernelcalls::EXCHANGEOSESSACK, &KernelcallHandler::exchangeOverSessionAck);
    ...
    add_operation(Kernelcalls::REVOKEBATCH, &KernelcallHandler::revokeBatch);
...
//            }
    ...
    case Kernelcalls::KFORWARD:
    {
        ...
        // Update membership table and re-send the request to the correct kernel.
        ...
        if(destKrnl != nullptr) {
            ...
            // Re-send the lock request to the correct kernel
            Kernelcalls::get().mhtlock(destKrnl, mht_key);
        }
        break;
    }
    }
...
void KernelcallHandler::createSessFwd(GateIStream &is) {
    ...
    if(srv == nullptr || srv->closing) {
        ...
        rgate->subscribe([this, rgate, rsrv, cap, tid, vpeID, sender] (GateIStream &reply, m3::Subscriber<GateIStream&> *s) {
             ...
             * We are inside notify_all's iteration over this gate's
...
void KernelcallHandler::createSessFail(GateIStream &is) {
    ...
    srv->removeChild(cap);
    srv->inst->rem_ref();
    Kernelcalls::get().reply(kpe);
}

/**
 * Remote-side handler for capability exchange over a session.
 *
 * This runs on the kernel that owns the service. It forwards the obtain/delegate
 * request to the service and processes the service's reply in a callback:
 *
 * For OBTAIN:
 *   1. Forward request to service
 *   2. Service replies with cap range
 *   3. Validate reply (type match, count check)
 *   4. For each cap: add child pointer to parent cap (for future revocation)
 *   5. Serialize parent cap data into exchangeOverSessionReply → requesting kernel
 *   6. Done — no ack needed from requesting kernel
 *
 * For DELEGATE:
 *   1. Forward request to service
 *   2. Service replies with destination cap range
 *   3. Reserve slots in service's CapTable (prevents races with concurrent exchanges)
 *   4. Send exchangeOverSessionReply → requesting kernel (with reserved slot info)
 *   5. Block thread (wait_for) until exchangeOverSessionAck arrives
 *   6. On ack: deserialize delegated caps from ack, insert into service CapTable
 *   7. On error: release reserved slots
 *
 * Thread usage: For obtain, the callback completes without blocking. For delegate,
 * one thread blocks waiting for the ack — this is the two-way handshake.
 */
...
//            }
            ...
            // put thread to sleep
            // For delegate, we block waiting for an ACK from the requesting kernel.
            // Track this pending delegate so it can be cleaned up if the peer disconnects.
            if(!obtain) {
                int mytid = m3::ThreadManager::get().current()->id();
                KernelcallHandler::get().addPendingDelegate(
                    new KernelcallHandler::PendingDelegate(mytid, sender));
                m3::ThreadManager::get().wait_for(reinterpret_cast<void*>(mytid));
                KernelcallHandler::get().removePendingDelegate(mytid);
                ...
                if(res != m3::Errors::NO_ERROR) {
                    ...
                    // Delegate: read the serialized caps from the ack and insert
                    // them into the service VPE's cap table.
                    size_t dataSize;
                    m3::Unmarshaller ackData(ack + m3::ostreamsize<m3::Errors::Code, unsigned long int>(),
                        *reinterpret_cast<const unsigned long int*>(ack + m3::ostreamsize<m3::Errors::Code>()));
                    uint capCount;
                    ackData >> capCount;

                    for(uint i = 0; i < capCount && i < srvcaps.count(); i++) {
                        Capability *delegatedCap = Capability::createFromStream(ackData);
                        if(delegatedCap) {
                            capsel_t dstsel = srvcaps.start() + i;
                            srvTbl.release(dstsel);
                            delegatedCap->put(&srvTbl, dstsel);
                            srvTbl.set(dstsel, delegatedCap);
                        }
                    }
                    // Release any remaining reserved slots
                    for(uint i = capCount; i < srvcaps.count(); i++)
                        srvTbl.release(srvcaps.start() + i);
                }
            }
        }
...
void KernelcallHandler::exchangeOverSessionAck(GateIStream &is) {
    int tid;
    m3::Errors::Code res;
    is >> tid >> res;
    LOG_KRNL(Coordinator::get().getKPE(is.label()), "kernelcall::exchangeOverSessionAck(tid=" <<
        tid << ", res=" << res << ")");
    if(res == m3::Errors::NO_ERROR && is.remaining()) {
        // For delegate: forward the cap data to the blocked thread
        AutoGateOStream reply(is.remaining() + m3::ostreamsize<m3::Errors::Code, unsigned long int>());
        reply << res << is.remaining();
        reply.put(is);
        m3::ThreadManager::get().notify(reinterpret_cast<void*>(tid),
...
void KernelcallHandler::removeChildCapPtr(GateIStream &is) {
    ...
    uint orphanedCount = 0;
    for(uint i = 0; i < parentCount; i ++) {
        const MHTItem &parentItem = MHTInstance::getInstance().get(parentStart + i);
        if(parentItem.data == nullptr) {
            // Parent may have been concurrently revoked. The child pointer is already
            // gone (revocation clears all children), so this is benign.
            KLOG(KRNLC, "removeChildCapPtr: parent " << PRINT_HASH(parentStart + i)
                << " not found (likely already revoked)");
            orphanedCount++;
            continue;
        }
    ...
    if(orphanedCount)
        KLOG(KRNLC, "removeChildCapPtr: " << orphanedCount << " of " << parentCount
            << " parent caps not found (orphaned child pointers already cleaned up by revocation)");
    Kernelcalls::get().reply(Coordinator::get().getKPE(is.label()));
}
...
/**
 * Remote revocation handler: revoke a single capability on behalf of another kernel.
 *
 * Called when a parent kernel encounters a remote child during revocation and sends
 * a REVOKE kernelcall to this kernel. This handler:
 *
 *   1. Looks up the capability in the MHT
 *   2. If found: calls CapTable::revoke() which recurses into revoke_rec()
 *   3. If not found (already revoked): checks RevocationList for in-flight revocation
 *      and subscribes if necessary
 *   4. If no outstanding remote sub-revocations: immediately sends revokeFinish(-1)
 *      to the parent kernel, decrementing its awaitedResp counter
 *   5. If outstanding: sends only a reply (ack). The revokeFinish will be sent later
 *      by the revokeFinish handler when all sub-revocations complete
 *
 * The awaits parameter in revokeFinish is -1 (decrement parent's counter by 1).
 */
...
/**
 * Batched remote revocation handler: revoke multiple capabilities in a single message.
 *
 * Same semantics as revoke() but processes multiple capIDs from one message. All
 * children targeted at this kernel are batched together by CapTable::revoke_rec().
 * If all sub-revocations complete synchronously, sends a single revokeFinish with
 * awaits = -count (decrements parent's counter by the batch size). Otherwise sends
 * only a reply; individual revokeFinish messages propagate as sub-revocations complete.
 */
void KernelcallHandler::revokeBatch(GateIStream &is) {
    mht_key_t parent, originCap;
    uint count;
    is >> parent >> originCap >> count;
    LOG_KRNL(Coordinator::get().getKPE(is.label()), "kernelcall::revokeBatch(parent=" <<
        PRINT_HASH(parent) << ", originCap=" << PRINT_HASH(originCap) <<
        ", count=" << count << ")");
    ...
    for(uint i = 0; i < count; i++) {
        ...
        totalAwaited += awaited;
    }

    if(!totalAwaited)
        Kernelcalls::get().revokeFinish(Coordinator::get().getKPE(is.label()), parent, -static_cast<int>(count), true);
    else
        Kernelcalls::get().reply(Coordinator::get().getKPE(is.label()));
}

/**
 * Revocation completion handler: processes acknowledgements from child revocations.
 *
 * When a remote kernel completes revoking a capability, it sends revokeFinish to
 * the parent's kernel. This handler:
 *
 *   1. Finds the Revocation entry in RevocationList for the initiator capID
 *   2. Adds `awaits` to awaitedResp (typically -1 for single, -N for batch)
 *   3. If awaitedResp reaches 0 and this is the revocation root (capID == origin):
 *      wakes the blocked root thread via notify(tid), which will clean up
 *   4. If awaitedResp reaches 0 and this is an intermediate node:
 *      notifies subscribers, sends revokeFinish upstream to parent kernel,
 *      and removes from RevocationList
 *
 * The includeReply flag indicates whether this message consumes a msg_received()
 * credit (true when it piggybacks on a reply, false when sent as a standalone notification).
 */
