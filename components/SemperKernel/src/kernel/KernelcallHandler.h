...
public:
    ...
    // Tracks a thread blocked in the delegate handshake waiting for an ACK.
    // Used to clean up leaked threads when a peer kernel disconnects.
    struct PendingDelegate : m3::SListItem {
        explicit PendingDelegate(int _tid, membership_entry::krnl_id_t _krnlId)
            : tid(_tid), krnlId(_krnlId) {}
        int tid;
    ...
    void addPendingDelegate(PendingDelegate *pd) {
        _pendingDelegates.append(pd);
    }
    void removePendingDelegate(int tid) {
        for(auto it = _pendingDelegates.begin(); it != _pendingDelegates.end(); it++) {
            if(it->tid == tid) {
                _pendingDelegates.remove(&*it);
                delete &*it;
                return;
            }
        }
    }
    // Wake all threads blocked waiting for delegate ACKs from a disconnected kernel.
    // Called during KPE removal to prevent permanent thread leaks.
    void cleanupDelegatesForKernel(membership_entry::krnl_id_t krnlId) {
        for(auto it = _pendingDelegates.begin(); it != _pendingDelegates.end(); ) {
            auto cur = it++;
            if(cur->krnlId == krnlId) {
                m3::Errors::Code err = m3::Errors::INV_ARGS;
                m3::ThreadManager::get().notify(reinterpret_cast<void*>(cur->tid),
                    reinterpret_cast<void*>(&err), sizeof(m3::Errors::Code));
                _pendingDelegates.remove(&*cur);
                delete &*cur;
            }
        }
    }
    ...
    void revokeBatch(GateIStream &is);
...
private:
    ...
    m3::SList<PendingDelegate> _pendingDelegates;
