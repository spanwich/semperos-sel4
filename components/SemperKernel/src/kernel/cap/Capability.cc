...
void Capability::addChild(mht_key_t child) {
    // Reject addChild if this capability is being revoked. The revocation has already
    // moved our children list, and the cap will be unset shortly. Any child added now
    // would become an orphan not covered by the ongoing revocation.
    if(_revoking) {
        KLOG(KRNLC, "addChild rejected: cap " << PRINT_HASH(_id) << " is being revoked");
        return;
    }
